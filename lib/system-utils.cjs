...
function inspectEnginePaths({ engineRoot, runnerExists = false, installerExists = false, moduleInstalled = false }) {
  const root = String(engineRoot);
  return {
    engineRoot: root,
    runnerScript: path.win32.join(root, 'run_transcribe.ps1'),
    installerScript: path.win32.resolve(root, '..', 'install_whisper_windows.bat'),
